...
{
	ll n = 0;
	for (ll i = b.length() - 1, j = 0; i >= 0; --i, ++j)
	{
		if (b[i] == '1')
		{
			n += (1 << j);
...
void dfs(ll s, vector<bool> &vs, vvll &v)
{
	if (vs[s])
		return;
	vs[s] = 1;
...
void bfs(ll s, vector<bool> &vs, vvll &v)
{
	queue<ll> q;
	q.push(s);
	vs[s] = 1;
...
void solution(ll &T){
	...
	ll ans=0;
	for(ll& i: v){
		if(i<0) less.push_back(i);
		else if(i>0) more.push_back(i);
