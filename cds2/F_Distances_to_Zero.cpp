...
void solution(ll &tc){
	ll n; cin>>n; vll v(n); vecin(v); vll ze;
	for (ll i = 0; i < v.size(); i++){
		if(v[i]==0) ze.push_back(i);
	}
	ll c=0;
	for (ll i = 0; i < n; i++){
		if(c==0 and i<=ze[c]){
			cout<<ze[c]-i<<' '; if(i==ze[c]) c++;
		}
		else if(c<ze.size()-1){
			cout<<( (i-ze[c-1])<=(ze[c]-i)?(i-ze[c-1]):(ze[c]-i) )<<' '; if(i==ze[c]) c++;
		}
		else cout<<i-ze[ze.size()-1]<<' ';
	}
