...
if(n==1){
		std::cout << count(s.begin(),s.end(),'0')<<endl; return;
	}
	for (ll i = 0; i < n; i++){
		if (s[i]=='0'){
			if(i==0 or i==n-1){
				if ((i == 0 and s[1] == '+') or (i == n - 1 and s[n-2] == '+')){
					s[i] = '+';
					continue;
				}
				else if ((i == 0 and s[1] == '-') or (i == n - 1 and s[n-2] == '-')){
					s[i] = '-';
					continue;
				}
			}

			if ((s[i+1]=='0' and s[i-1]=='+')or(s[i+1]=='+' and s[i-1]=='+')or(s[i+1]=='+' and s[i-1]=='0'))
				s[i]='+';
			else if ((s[i+1]=='0' and s[i-1]=='-')or(s[i+1]=='-' and s[i-1]=='-')or(s[i+1]=='-' and s[i-1]=='0'))
				s[i]='-';
		}
	}
	std::cout << count(s.begin(),s.end(),'0')<<endl;