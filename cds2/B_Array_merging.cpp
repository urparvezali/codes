...
void solution(ll &T){
	ll n; cin>>n;vll v1(n), v2(n);map<ll, ll> m1, m2;vecin(v1);vecin(v2);ll c=1;v1.push_back(LONG_MIN);v2.push_back(LONG_MIN);for (ll i = 0; i < n; i++){if(v1[i]==v1[i+1]){c++;}else{m1[v1[i]]=max(m1[v1[i]],c);c=1;}}c=1;for (ll i = 0; i < n; i++){if(v2[i]==v2[i+1]){c++;}else{m2[v2[i]]=max(m2[v2[i]],c);c=1;}}ll mx=0;for(auto &i:m1){mx=max(i.second+m2[i.first],mx);}for(auto &i:m2){mx=max(i.second+m1[i.first],mx);}cout<<mx<<endl;
}
