...
void solution(ll &tc){
	char c[4][4];for(ll i=0;i<4;i++)for(ll j=0;j<4;j++)cin>>c[i][j];ll t=0;for(ll i=0;i<3;i++)for(ll j=0;j<3;j++){if(c[i][j]=='#')t++;if(c[i][j+1]=='#')t++;if(c[i+1][j]=='#')t++;if(c[i+1][j+1]=='#')t++;if(t!=2){Y();return;}t=0;}N();
}
