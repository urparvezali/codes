...
void solution(ll &tc){
	ll n; cin>>n; vll v(n); vecin(v);
	ll last=0,fi=0;
	for (ll i = 0; i < n; i++){
		if(v[i]==2) last=i;
	}
	ll firstn=count(v.begin(),v.begin()+last,1),lastn= count(v.begin()+last,v.end(),1);
	
	ll male=firstn/2,female; if(firstn&1) female=firstn/2+1; else female=male;
	
	if(male%2==0) fi+=male/2;
	else fi+=male/2+1;
	if(female%2==0) fi+=female/2;
	else fi+=female/2+1;
