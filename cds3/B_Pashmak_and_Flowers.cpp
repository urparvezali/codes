...
void dfs(ll n,vector<bool>& vs, vector<vector<ll>>& v){if(vs[n])return;vs[n]=1;for(auto& i:v[n])dfs(i,vs,v);}


void solution(ll &T){
	ll n; input(n);
	vll v(n);ll mx=0,mn=LONG_MAX;
	for (ll i = 0; i < n; i++)
	{
	...
	if(mx==mn){
		print(mx-mn,(cmx)*(cmx-1)/2);return;
	}
	print(mx-mn,cmn*cmx);
}
