...
void solution(ll &T){
	...
	input(px,py,ax,ay,bx,by);
	double dab=sqrt((ax-bx)*(ax-bx)+(ay-by)*(ay-by));
	double dpa=sqrt((ax-px)*(ax-px)+(ay-py)*(ay-py));
	double dpb=sqrt((bx-px)*(bx-px)+(by-py)*(by-py));
	double za=sqrt((ax)*(ax)+(ay)*(ay));
	double zp=sqrt((px)*(px)+(py)*(py));
	double zb=sqrt((bx)*(bx)+(by)*(by));
	
	double ans=LLONG_MAX;

	if(dpa<=dab/2 or dpb<=dab/2){
		ans=min(dab/2,ans);
	}
	if(dab<=dpa*2){
		ans=min(dpa,ans);
