...
void dijkstra(ll s, vll &dist, const vector<vpll> &g,vll& parent){
	ll n = g.size();
	dist[s] = 0;
...
void solution(ll &T){
	ll v,e; input(v,e);
	vector<vpll> g(v+1,vpll ());
	for (ll i = 0; i < e; i++)
	{
		ll x,y,w; input(x,y,w);
		g[x].push_back({y,w});g[y].push_back({x,w});
	}
	vll dist(v+1,LLONG_MAX),parent(v+1);
	dijkstra(1,dist,g,parent);
	if(dist[v]==LLONG_MAX) MINUS();
	else{
		ll temp=v;
		vll res;
		while(temp){
