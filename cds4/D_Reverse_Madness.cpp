...
void solution(ll &T){
	...
	for (int i = 0; i < s.length(); ++i){
		...
		while (l >= 0 && r < s.length() && s[l] == s[r]){
			if(r-l>1){
				pal[make_pair(l,r)]=true;
				// print(s.substr(l,r-l+1));
			}
			--l;
			++r;
		}
	...
	for(auto i:mp){
		...
		ll le=(i.first).first; ll ri=(i.first).second;
		if(le<ri){
			if(pal[make_pair(le,ri)]) continue;
			reverse(s.begin()+le-1,s.begin()+ri);
		}
	}
	print(s);
}
