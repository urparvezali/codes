#include <bits/stdc++.h>
using namespace std;

typedef long long ll;
typedef pair<ll,ll> pll;
typedef vector<bool> vbl;
typedef vector<ll> vll;
typedef vector<vll> vvll;
typedef vector<string> vstr;
typedef vector<pll> vpll;

#define endl '\n'
#define all(x) x.begin(),x.end()
constexpr ll mod = 1000000007;

inline void Parvez(){ ios_base::sync_with_stdio(0);cin.tie(0);cout.tie(0); }
inline void print(){ cout<<endl; }template<class H,class... T>inline void print(H&& h,T &&...t){ cout<<h;if(sizeof...(t)!=0)cout<<' ';print(forward<T>(t)...); }template<class T>inline void print(vector<T>& v){ for(auto& i:v){ cout<<i;if(&i!=&v.back())cout<<' '; }cout<<endl; }inline void input(){}template<class H,class... T>inline void input(H&& h,T &&...t){ cin>>h;input(forward<T>(t)...); }template<class T>inline void input(vector<T>& v){ for(auto& i:v)cin>>i; }

void rec(ll s,vector<pair<vll,ll>>& v,ll par,vll& childs,vll& level,vll& parents){

	for(auto& u:v[s].first){
		if(u!=par){
			parents[u]=s;
			level[u]=level[s]+1;
			if(v[u].first.size()==1){
				childs.push_back(u);
				v[u].second=1;
				v[s].second+=v[u].second;
			} else {
				rec(u,v,s,childs,level,parents);
				v[s].second+=v[u].second;
			}
		}
	}
}


void solution(ll& T){
	ll n; input(n); vll childs; vll level(n+1); vll parents(n+1);
	vector<pair<vector<ll>,ll>> v(n+1);
	for(ll i = 0,x,y; i < n-1; i++){
		input(x,y);
		v[x].first.push_back(y);
		v[y].first.push_back(x);
	}
	rec(1,v,0,childs,level,parents);

	// for(ll i = 0; i < 7; i++){
	// 	print(i,">",v[i].second,">");print(v[i].first);
	// }

	// print(v[1].second);


	ll ans=0;
	for (ll child:childs){
		ll jaite=level[child]/2;
		jaite;
		while (jaite){
			child=parents[child];
			jaite--;
		}
		ans=max(ans,v[child].second);
	}
	print(ans);
}

int32_t main(){
	Parvez();
	ll TT=1;
	cin>>TT;
	for(ll T=1;T<=TT;T++)
		solution(T);
	return 0;
}