...
int comp(pll a,pll b){return a.first>b.first;}

void solution(ll &T){
	ll n,m,k; input(n,m,k);
	vll j(n),g(m); input(j); input(g);
	sort(all(j)); sort(all(g));
	multiset<ll> jj(all(j)),gg(all(g));

	ll mn1=*jj.begin(), mn2=*gg.begin(), mx1=*jj.rbegin(), mx2=*gg.rbegin();
	if(k>50){
		if(k&1) k=51;
		else k=52;
	}
	for (ll i = 1; i <= k; i++){
		mn1=*jj.begin(), mn2=*gg.begin(), mx1=*jj.rbegin(), mx2=*gg.rbegin();
		if(i%2==1){
			if(mn1<mx2){
				jj.erase(jj.begin());
				jj.insert(mx2);
