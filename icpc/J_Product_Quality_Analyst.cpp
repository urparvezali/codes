...
void solution(){
	ll tc; cin >> tc;
	for (ll i = 1; i <= tc; i++){
		ll n, k; cin >> n>> k; vll v(n); vecin(v);
		ll timeforone = accumulate(v.begin(), v.end(), 0);
		if (k == 1){
			cout << "Case " << i << ": " << timeforone << endl;
		}
		else
			cout << "Case " << i << ": " << timeforone + (k-1)*(*max_element(v.begin(), v.end())) << endl;
	}
}
