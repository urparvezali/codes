...
void solution(ll tc){
	...
	for (ll i = 0; i < 100000; i++){
		if (isokay(v[0], v[1], v[2])){cout << "Case " << tc << ": "<< "Peaceful" << endl;return;}

		if (*max_element(v.begin(), v.end()) >= k){
		*max_element(v.begin(), v.end()) -= k;
		*min_element(v.begin(), v.end()) += k;
		}
		else {cout << "Case " << tc << ": "<< "Fight" << endl;return;}
	}
