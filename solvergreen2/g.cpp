...
class Sieve{vector<bool> vp;public:Sieve(ll np){vp.resize(np+1,true);vp[0]=vp[1]=false;for(ll i=2;i<=np;i++){if(vp[i]==true){for(ll j=2*i;j<=np;j+=i){vp[j]=false;}}}}bool getprime(ll n){return vp[n];}};
vector<ll> segmentedSieve(ll L, ll R){ll lim=sqrt(R);vector<bool>mark(lim+1,false);vector<ll>primes;for(ll i=2;i<=lim;++i)if(!mark[i]){primes.push_back(i);for(ll j=i*i;j<=lim;j+=i)mark[j]=true;}vector<bool>isPrime(R-L+1,true);for(ll p:primes)for(ll j=max(p*p,(L+p-1)/p*p);j<=R;j+=p)isPrime[j-L]=false;vector<ll>result;for(ll i=L;i<=R;++i)if(isPrime[i-L])result.push_back(i);return result;}
...
void solution(ll &T){
	...
	string x1=ans[0],y1=ans[4],x2=ans[1],y2=ans[3];
	reverse(all(y1));
	reverse(all(y2));
	string ttt=ans[2];
	reverse(all(ttt));
	if(x1==y1 && x2==y2 && ans[2]==ttt){
		print("TRUE");
	}else print("FALSE");
}
